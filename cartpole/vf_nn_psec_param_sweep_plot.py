...
def plot_data(data, file_name, plot_params):
    ...
    for method in sorted(data):
        ...
        if method == 'TD(0)':
            line, = plt.plot(x, y, label=label,  linewidth = 5, color = 'red')
        else:
            line, = plt.plot(x, y, label=label,  linewidth = 5)
...
def best_lr(data):
                                ...
                                for psec_nh in data[method][num_traj][lr][ti][t1][t2][psec_act]:
                                    for psec_ne in data[method][num_traj][lr][ti][t1][t2][psec_act][psec_nh]:
                                        for psec_lr in data[method][num_traj][lr][ti][t1][t2][psec_act][psec_nh][psec_ne]:                           
                                            ...
                                            mean = np.mean(errs)
                                            meth_stats[method + '_' + psec_nh + '_' + str(num_traj)] = errs
                                            if psec_nh == '3':
    ...
    for m in meth_stats:
        if 'PSEC' in m:
            sp = m.split('_')[1]
            tr = m.split('_')[2]
            print('other method {} and res {}'.format(m, stats.ttest_ind(meth_stats['TD(0)_0_' + tr],  meth_stats[m], equal_var = False)))
