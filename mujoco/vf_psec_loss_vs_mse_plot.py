...
def plot_losses(data, file_name, plot_params):
    ...
    for method in data:
        ...
        tr_mean = np.mean(tr_losses, axis = 0)#[:150]
        tr_std = np.std(tr_losses, axis = 0)#[:150]
        val_mean = np.mean(val_losses, axis = 0)#[:150]
        val_std = np.std(val_losses, axis = 0)#[:150]
        m = len(tr_mean) 
        x = [i * 25* 2 for i in range(m)]
...
def plot_mses(data, file_name, plot_params):
    ...
    for method in data:
        ...
        n = len(mses)
        mean = np.mean(mses, axis = 0)#[:200]
        std = np.std(mses, axis = 0)#[:200]
        m = len(mean)
        x = [i * 25 * 2 for i in range(m)]
        #mean = mean[2:]
        #std = std[2:]
        ...
        print ('method {} mse avged vs. steps {} and (95) error {}, num trials {}'.format(method, mean, yerr, n))
        ylower = mean - yerr
        yupper = mean + yerr
