...
def run_trial(seed,
    ...
    if condor:
        ...
        submitFile += 'Requirements = ARCH == "X86_64" && !GPU && (Angrist || !InMastodon)\n'
        #submitFile += 'Requirements = ARCH == "X86_64" && !GPU && InMastodon\n'
...
def main():  # noqa
    ...
    # lr goes up to -14
    vf_act_fn = [0]
    vf_num_hidden_layers = [2] # HACKY BUT IMPORTANT, check if statement in loops
    #vf_num_hidden_layers =[2]#[4] # HACKY BUT IMPORTANT, check if statement in loops
    vf_num_neurons = [64]#[128]
   
    psec_act_fns = [0]
    #psec_num_hidden_layers = [2, 3, 4]
    #psec_num_hidden_layers = [1,2,3]
    psec_num_hidden_layers = [2]
    psec_num_neurons = [64]
    methods = []
    ...
    for m in range(start_num_trajs, end_num_trajs + 1, num_trajs_increment):
        for vf_act in vf_act_fn:
            #vf_alphas = [(0.01) * 2 ** i for i in range(-5,0)]
            #print (vf_alphas)
            vf_alphas = [0.01 * 2 ** i for i in range(-6,1)]
            #vf_alphas = [1 * 2 ** i for i in range(-1,1)]
            #vf_alphas = [0.5, 0.75, 1.0]
            #vf_alphas = [0.01 * 2 ** i for i in range(-8,0)]
            vf_alphas = [1.0]
            for a in vf_alphas:
                        ...
                        for seed in seeds:
                            for method in methods:
                                if 'PSEC' in method.name:
                                    psec_alphas = [0.1 * 2 ** i for i in range(-8,2)]
                                    psec_alphas = psec_alphas + [0.003, 0.001, 0.0003]
                                    psec_alphas = [0.000781]
                                    print (psec_alphas)
